#!/bin/bash
#SBATCH --job-name=make_bigwigs      ## Name of the job.
#SBATCH -A ecoevo283       ## account to charge 
#SBATCH -p standard          ## partition/queue name
#SBATCH --nodes=1            ## (-N) number of nodes to use
#SBATCH --array=1-25         ## number of tasks to launch
#SBATCH --cpus-per-task=2    ## number of cores the job needs
#SBATCH --output=make_bigwigs-%J.out ## output log file
#SBATCH --error=make_bigwigs-%J.err ## error log file

module load samtools/1.10
module load ucsc-tools/v393 

inpath="/data/class/ecoevo283/erebboah/ATACseq/"
ref="/data/class/ecoevo283/erebboah/ref/dm6.chrom.sizes"
